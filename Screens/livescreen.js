...
import { View, Text, StyleSheet, FlatList, Image, TouchableOpacity, ActivityIndicator, RefreshControl, StatusBar, Platform, Dimensions, DeviceEventEmitter } from 'react-native';
...
const LIVE_QUERIES = [
  "bangladesh live tv channel 24/7", "india live tv channel hindi news", "pakistan live tv channel news", "live sports channel 24/7", "live movie tv channel"
];

const TOP_BAR_QUERIES = [
  "bangladesh live tv channel 24/7", "bangladesh news live stream", "india live tv channel hindi news", "indian regional news live",
  "pakistan live tv channel news", "live sports channel 24/7", "world news live stream", "live entertainment tv channel", "live gaming stream"
];
...
export default function LiveScreen() {
  ...
  const fetchTopChannels = async (queryIndex) => {
            ...
            if (node.videoRenderer) {
                ...
                if (channelName && channelId && liveVideoId) {
                    if (!newChannels.some(c => c.id === channelId)) {
                        newChannels.push({ id: channelId, name: channelName, logo: finalAvatar, channelUrl: channelUrl, liveVideoId: liveVideoId });
                    }
                }
    ...
    } catch (e) {} finally { setIsFetchingTopChannels(false); }
  };
      ...
      if (match && match[1]) {
        ...
        const formattedVideos = extractedVideos.map(vid => {
            ...
            return {
              ...
              channelId: vid.ownerText?.runs?.[0]?.navigationEndpoint?.browseEndpoint?.browseId || '', channelUrl: channelUrl,
              ...
              thumbnail: getHighQualityThumbnail(vid.thumbnail, vid.videoId), avatar: finalAvatar
            };
        });
        setVideos(isNewSearch ? formattedVideos : [...videos, ...formattedVideos]);
      }
    } catch (e) {} finally { setLoading(false); setRefreshing(false); setIsFetchingMore(false); }
  };
  ...
  const playTopChannelLive = (channel) => {
    const doScan = videoScanSettings[channel.liveVideoId] !== undefined ? videoScanSettings[channel.liveVideoId] : masterVideoScan;
    navigation.navigate('Player', { 
      videoId: channel.liveVideoId, 
      videoData: { id: channel.liveVideoId, title: channel.name, channel: channel.name, views: 'Live Now' },
      aiScanEnabled: doScan 
    });
  };
      ...
      <View style={styles.videoCard}>
        ...
        <View style={styles.videoInfo}>
          ...
          <View style={styles.textContainer}>
            <TouchableOpacity activeOpacity={0.8} onPress={() => navigateToPlayer(item)}>
          ...
          ListHeaderComponent={
            <View style={styles.topChannelsContainer}>
              {topChannels.length === 0 && isFetchingTopChannels ? (
                ...
                <FlatList
                  horizontal showsHorizontalScrollIndicator={false} data={topChannels}
                  keyExtractor={(item, index) => item.id + index.toString()}
                  renderItem={renderTopChannel} contentContainerStyle={{ paddingHorizontal: 8 }}
                  onEndReached={loadMoreTopChannels} onEndReachedThreshold={0.5}
                  ListFooterComponent={isFetchingTopChannels && topChannels.length > 0 ? (<View style={{ justifyContent: 'center', paddingHorizontal: 15 }}><ActivityIndicator size="small" color="#FF0000" /></View>) : null}
                />
              )}
            </View>
          }
...
function getDynamicStyles(isDark) {
  return StyleSheet.create({
    container: { flex: 1, backgroundColor: isDark ? '#0F0F0F' : '#F9F9F9', paddingTop: height / 70 },
